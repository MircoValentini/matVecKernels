...
for (( i=${1}; i<=${2}; i++ )) ; do
	for (( j=0; j<${N}; j++ )) ; do
		...
		newAlg=`echo ${algorithms[${j}]} | sed -e 's/BlockRowPerm/BlockRowPermCC/'`;
                sbatch --time ${time}  submitMatVec.sh ${id} ${newAlg}
	done
done
